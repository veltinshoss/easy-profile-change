...
import static de.pepping.android.ringtone.handler.apn.ApnControl.STATE_OFF;
...
import de.pepping.android.ringtone.handler.apn.ApnControl;
...
public class MobileDataSettingHandler2 extends SettingHandler {
	...
	public void onSwitched(boolean enable) {
		...
		int perm = mActivity.checkCallingOrSelfPermission("android.permission.MODIFY_PHONE_STATE");
		System.out.println("android.permission.MODIFY_PHONE_STATE: " + (PackageManager.PERMISSION_DENIED == perm ? "denied" : "granted"));

		perm = mActivity.checkCallingOrSelfPermission("android.permission.FLASHLIGHT");
		System.out.println("android.permission.FLASHLIGHT: " + (PackageManager.PERMISSION_DENIED == perm ? "denied" : "granted"));
	...
	void updateState(State state) {
		...
		switch (state) {
			...
			default: s.checked = s.enabled = false; resId = R.string.txt_no_apn; break;
		}
